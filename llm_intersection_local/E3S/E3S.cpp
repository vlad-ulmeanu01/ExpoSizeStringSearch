...
ExpoSizeStrSrc::ExpoSizeStrSrc() {
    stores_dag_info = true;

    logOtp = new std::pair<uint64_t, uint64_t>[ml2];
    subOtp = new std::pair<uint64_t, uint64_t>[ml2];
    basePow = new std::pair<int64_t, int64_t>[maxn+1];
    hhPref = new std::pair<int64_t, int64_t>[maxn+1];
    hash = new uint64_t[(1<<ml2)*ml2];
    subtreeHash = new std::pair<uint64_t, int>[(1<<ml2)*ml2];
    id = new int[(1<<ml2)*ml2];
    leverage = new int[(1<<ml2)*ml2];
    starterNodeChildren = new int[maxn*ml2];
...
ExpoSizeStrSrc::ExpoSizeStrSrc(ExpoSizeStrSrc *oth) {
    ///used when only wanting a different trie & using another class instance's DAG.
    stores_dag_info = false;
    ...
    hhPref = oth->hhPref;
    hash = oth->hash;
    subtreeHash = oth->subtreeHash;
    id = oth->id;
    leverage = oth->leverage;
    starterNodeChildren = oth->starterNodeChildren;
    
    cntStarterNodeChildren = oth->cntStarterNodeChildren;
    n = oth->n;
}
...
void ExpoSizeStrSrc::updateText(const std::vector<uint8_t> &newS, int lengthNewS) {
    ...
    std::sort(starterNodeChildren, starterNodeChildren + cntStarterNodeChildren); ///the starter node's children aren't already sorted.
 ...
 * @param stop_after_first_insert: if we expect to not find the majority of the strings in the dictionary, we should add only the first link of the chain,
          then redo the trie only with the strings that were able to be propagated.
 */
void ExpoSizeStrSrc::insertQueriedString(const std::vector<uint8_t> &t, int lengthT, bool stop_after_first_insert) {
    ...
    for (i = ml2, z = 0; i >= 0; i--) {
        if (lengthT & (1<<i)) {
            ...
            if (stop_after_first_insert) break;
        }
    }

    trieNow->indexesEndingHere.push_back((int)massSearchResults.size() - 1); ///TODO treci in indexEndingHere. nu ne pasa decat de un match.
}
...
void ExpoSizeStrSrc::massSearch(TrieNode *trieNow) {
    ...
    for (int x: trieNow->indexesEndingHere) { ///more indexes means duplicate strings in the dictionary.
    ...
    std::vector<std::pair<uint64_t, TrieNode *>> sons(cntSons);
    ...
    for (auto &x: trieNow->idLevsCurrentlyHere) {
        ...
        if (dagNode == -1) { ///am in the starter node.
            ...
            dagNodeP2 = 1 << (dagNode >> ml2);
            dagNodeStartInd = dagNode - (dagNode >> ml2) * (1 << ml2);
            m = 0;
        ...
        for (i = 0; i < m; i++) {
            ...
            if (dagNode == -1) {
                ...
                p2 = 1 << (nn >> ml2); ///length of the string associated to nn.
                startInd = nn - (nn >> ml2) * (1 << ml2); ///the index at which the associated string of nn begins in s.
                ...
                hh_red = reduceHash(hh, logOtp[nn >> ml2]);
                hash[nn] = hh_red; ///keep the value. might use it later in the search.
            } else {
                nn = id[(1 << ml2) * i + dagNodeStartInd + dagNodeP2]; ///we're sure that id[dagNode] = dagNode. it doesn't necessarily mean that id[nn] = nn in this case.
                hh_red = hash[nn]; ///hash was already precalculated when extending from the starter node.
            }

            auto it = std::lower_bound(sons.begin(), sons.end(), std::make_pair(hh_red, nullptr),
                                       [](const std::pair<uint64_t, TrieNode *> &a, std::pair<uint64_t, TrieNode *> b) {
                                           return a.first < b.first;
                                       });

            if (it != sons.end() && it->first == hh_red) {
...
void ExpoSizeStrSrc::trieBuffersFree() {
    TNBufInd = TNBufSz; ///need to reset this to make sure the next alloc call doesn't try a deallocated chunk.
