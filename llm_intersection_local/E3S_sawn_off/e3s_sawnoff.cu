...
E3S_sawnoff::E3S_sawnoff():
    mt(DEBUG_FLAG? DEBUG_SEED: rd()),
    base(std::uniform_int_distribution<uint64_t>(257, M61 - 1)(mt))
{}
...
void E3S_sawnoff::read_ts(std::unique_ptr<DictReader> dire) {
    ...
    auto flush_streaming_segment = [this, &sseg_m, &sseg_start_i, &hst_sseg_ts_buff, &hst_set_keys_at, &hst_ts_info](int sseg_end_i) {
...
void E3S_sawnoff::compute_ts_counts() {
